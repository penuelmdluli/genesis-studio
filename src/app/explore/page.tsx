...
export default function ExplorePage() {
  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12 text-center">
          ...
          <p className="mt-5 text-sm sm:text-base text-zinc-400 max-w-xl mx-auto">
            Every video on this page was generated by AI in under 2 minutes.
            <br className="hidden sm:block" />
            No cameras. No editing. Just a prompt.
          </p>
          ...
          {!isSignedIn && (
            <div className="mt-8 flex flex-col sm:flex-row gap-3 justify-center items-center">
              ...
              >
                ...
                Create Your First Video — 100 Free Credits
          ...
          {isSignedIn && (
            <Link
              href="/generate"
              className={cn(
                "inline-flex items-center gap-2 mt-8 px-5 py-2.5 rounded-xl",
                "bg-white/[0.08] hover:bg-white/[0.12] border border-white/[0.10]",
      ...
      {/* ========== Social Proof Bar ========== */}
      {!isSignedIn && (
        <div className="border-y border-white/[0.10] bg-white/[0.02]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex flex-wrap justify-center gap-8 text-center">
              <div>
                <p className="text-lg font-bold text-zinc-200">100</p>
                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">Free Credits</p>
              </div>
              <div>
                <p className="text-lg font-bold text-zinc-200">60s</p>
                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">Per Video</p>
              </div>
              <div>
                <p className="text-lg font-bold text-zinc-200">10+</p>
                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">AI Models</p>
              </div>
              <div>
                <p className="text-lg font-bold text-zinc-200">Lip Sync</p>
                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">Characters Speak</p>
              </div>
              <div>
                <p className="text-lg font-bold text-zinc-200">Auto</p>
                <p className="text-[10px] text-zinc-400 uppercase tracking-wider">Voice + Music + Captions</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            {videos.map((video, idx) => (
              <>{/* eslint-disable-next-line react/jsx-key */}
                ...
                />
                {/* Inline CTA after every 6th video for non-signed-in users */}
                {idx === 5 && !isSignedIn && (
                  <div key="cta-inline" className="col-span-full rounded-2xl border border-violet-500/20 bg-gradient-to-r from-violet-500/10 via-transparent to-cyan-500/10 p-6 sm:p-8 text-center my-2">
                    <h3 className="text-lg sm:text-xl font-bold text-zinc-100 mb-2">You can make videos like these</h3>
                    <p className="text-sm text-zinc-400 mb-4 max-w-md mx-auto">
                      Type a prompt. Pick a style. Get a cinematic video with voiceover, music, and captions — in 60 seconds.
                    </p>
                    <Link
                      href="/sign-up"
                      className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-violet-600 to-violet-500 text-white font-semibold text-sm shadow-lg shadow-violet-600/25 hover:shadow-violet-500/35 transition-all"
                    >
                      <Sparkles className="w-4 h-4" />
                      Try It Free — 100 Credits
                    </Link>
                  </div>
                )}
              </>
